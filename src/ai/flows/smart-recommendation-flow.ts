...
const SmartRecommendationsInputSchema = z.object({
  priceRange: z.string().describe('The desired price range for the car rental.'),
  ...
  location: z.string().describe('The location where the car rental is required'),
});
...
const SmartRecommendationsOutputSchema = z.object({
  recommendations: z.array(
    z.object({
      ...
      price: z.number().describe('The price per day of the rental.'),
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an AI assistant specialized in providing smart car rental recommendations.

  Based on the user's specified preferences, provide a list of car rental options that best match their needs.
  Consider the price range, car type, desired features, and rental location.
